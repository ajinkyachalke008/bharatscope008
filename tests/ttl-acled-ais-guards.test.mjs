...
describe('cache TTL alignment with upstream refresh rates', () => {
  it('climate anomalies uses 3h TTL (ERA5 has 2-7 day lag)', () => {
    ...
    assert.match(src, /REDIS_CACHE_TTL = 10800/, 'Climate cache TTL should be 10800s (3 hours)');
  });
  ...
  it('fire detections uses 1h TTL (FIRMS NRT updates every ~3h)', () => {
    ...
    assert.match(src, /REDIS_CACHE_TTL = 3600/, 'Fire cache TTL should be 3600s (1 hour)');
  });
});
...
describe('ACLED consumers use shared cache layer', () => {
  ...
  it('unrest handler imports fetchAcledCached', () => {
    ...
    assert.match(src, /fetchAcledCached/, 'Unrest handler should use shared ACLED fetch');
  });
...
describe('maritime AIS visibility guard', () => {
  ...
  it('skips polling when document is hidden', () => {
    assert.match(src, /document\.hidden/, 'Should check document.hidden');
    ...
    assert.ok(
      hiddenGuard > -1 && hiddenGuard < 400,
      'document.hidden check should be near the top of pollSnapshot',
    );
  });

  it('has pausePolling function that clears interval', () => {
    assert.match(src, /function pausePolling\(\)/, 'Should define pausePolling');
    const pauseFn = src.slice(
      src.indexOf('function pausePolling'),
      src.indexOf('function pausePolling') + 200,
    );
    assert.match(
      pauseFn,
  ...
  it('has resumePolling function that restarts polling without overlap', () => {
    assert.match(src, /function resumePolling\(\)/, 'Should define resumePolling');
  ...
  it('calls pausePolling on hidden and resumePolling on visible', () => {
    ...
    assert.match(listenerBlock, /document\.hidden/, 'Listener should check document.hidden');
    assert.match(listenerBlock, /pausePolling\(\)/, 'Should call pausePolling when hidden');
    assert.match(listenerBlock, /resumePolling\(\)/, 'Should call resumePolling when visible');
  });
});
