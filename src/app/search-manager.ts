...
export class SearchManager implements AppModule {
  ...
  private setupSearchModal(): void {
    ...
    this.ctx.searchModal.registerSource(
      ...
      INDIA_WEBCAMS.map((w) => ({
        id: w.name,
        title: w.name,
        subtitle: `Public Webcam • ${w.location || ''}`,
        data: w,
      }))
    );
    ...
    this.ctx.searchModal.registerSource(
      'indiarailwaystation',
      INDIA_RAILWAY_STATIONS.map((r) => ({
        id: r.code || r.name,
        title: `${r.name} (${r.code})`,
        subtitle: `Railway Station${r.platforms ? ` • ${r.platforms} Platforms` : ''}`,
        data: r,
      }))
    );
    ...
    this.ctx.searchModal.registerSource(
      ...
      INDIA_AIRPORTS.map((a) => ({
        id: a.iata || a.name,
        title: `${a.name} (${a.iata})`,
        subtitle: `Airport • ${a.location} • ${a.traffic}M Pax`,
        data: a,
      }))
    );
    ...
    this.ctx.searchModal.registerSource(
      'indianuclear',
      INDIA_NUCLEAR_PLANTS.map((n) => ({
        id: n.name,
        title: n.name,
        subtitle: `Nuclear Plant • ${n.location} • ${n.capacity}`,
        data: n,
      }))
    );
    ...
    this.ctx.searchModal.registerSource(
      'indiapowerplant',
      INDIA_POWER_PLANTS.map((p) => ({
        id: p.name,
        title: p.name,
        subtitle: `${p.type} Power Plant • ${p.location} • ${p.capacity}`,
        data: p,
      }))
    );
    ...
    this.ctx.searchModal.registerSource(
      'indiadam',
      INDIA_MAJOR_DAMS.map((d) => ({
        id: d.name,
        title: d.name,
        subtitle: `Major Dam • ${d.location} • ${d.height}`,
        data: d,
      }))
    );
    ...
    this.ctx.searchModal.registerSource(
      'indiatelecom',
      INDIA_DATA_CENTERS.map((d) => ({
        id: d.name,
        title: d.name,
        subtitle: `Data Center Hub • ${d.location} • ${d.notes}`,
        data: d,
      }))
    );
    ...
    this.ctx.searchModal.registerSource(
      'indiahsr',
      [{
        id: INDIA_HIGH_SPEED_RAIL.name,
        title: INDIA_HIGH_SPEED_RAIL.name,
        subtitle: `High Speed Rail Corridor`,
    ...
    this.ctx.searchModal.registerSource(
      ...
      INDIA_CORRIDORS.map((c) => ({
        id: c.name,
        title: c.name,
        subtitle: `Expressway Corridor • ${c.length}`,
        data: c,
      }))
    );
  ...
  private handleSearchResult(result: SearchResult): void {
    ...
    switch (result.type) {
      ...
      case 'indiawebcam':
      case 'indiarailwaystation':
      case 'indiaairport':
      case 'indianuclear':
      case 'indiapowerplant':
      case 'indiadam':
      case 'indiatelecom': {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const item = result.data as any;
        this.ctx.map?.setView('india');
        let layerName = '';
        if (result.type === 'indiawebcam') layerName = 'indiaWebcams';
        if (result.type === 'indiarailwaystation') layerName = 'indiaRailways';
        if (result.type === 'indiaairport') layerName = 'indiaAirports';
        if (result.type === 'indianuclear' || result.type === 'indiapowerplant' || result.type === 'indiadam') layerName = 'indiaEnergy';
        if (result.type === 'indiatelecom') layerName = 'indiaTelecom';
        if (layerName) {
          this.ctx.map?.enableLayer(layerName as keyof MapLayers);
          this.ctx.mapLayers[layerName as keyof MapLayers] = true;
        }
        setTimeout(() => {
          this.ctx.map?.setCenter(item.coordinates[1], item.coordinates[0], 6);
        }, 300);
        break;
      }
      case 'indiahsr':
      case 'indiacorridor': {
        ...
        const item = result.data as any;
        this.ctx.map?.setView('india');
        this.ctx.map?.enableLayer('indiaRailways');
        this.ctx.mapLayers.indiaRailways = true;
        setTimeout(() => {
          const pt = item.path[0];
          this.ctx.map?.setCenter(pt[1], pt[0], 6);
        }, 300);
        break;
      }
      case 'country': {
      ...
      case 'layers': {
        if (action === 'all') {
          ...
          if (preset) {
            for (const key of Object.keys(this.ctx.mapLayers))
              this.ctx.mapLayers[key as keyof MapLayers] = false;
            for (const layer of preset) this.ctx.mapLayers[layer] = true;
          }
        }
