// Dynamic Meta Tags Service for Bharat Scope
...
export function updateMetaTagsForStory(meta: StoryMeta): void {
  ...
  const title = `${countryName} Intelligence Brief | Bharat Scope`;
...
export function resetMetaTags(): void {
  const defaultTitle = 'Bharat Scope - Global Situation with AI Insights';
  const defaultDesc =
...
function generateDescription(
  ...
  return `Bharat Scope ${parts.join('. ')}. Free, open-source geopolitical intelligence.`;
}
