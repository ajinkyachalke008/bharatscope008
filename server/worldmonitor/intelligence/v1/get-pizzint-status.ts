...
export async function getPizzintStatus(
  ...
  const result = await cachedFetchJson<GetPizzintStatusResponse>(
    cacheKey,
    REDIS_CACHE_TTL,
    async () => {
      ...
      try {
        ...
        if (raw.success && raw.data) {
          const locations: PizzintLocation[] = raw.data.map((d) => ({
            ...
            dataFreshness: (d.data_freshness === 'fresh'
              ? 'DATA_FRESHNESS_FRESH'
              : 'DATA_FRESHNESS_STALE') as DataFreshness,
        ...
        /* PizzINT unavailable — continue to GDELT */
      }
